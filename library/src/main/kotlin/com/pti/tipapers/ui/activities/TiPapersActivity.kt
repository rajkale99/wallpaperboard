...
import com.pti.tipapers.data.models.db.FavoritesDatabase
import com.pti.tipapers.helpers.extensions.showChanges
import com.pti.tipapers.helpers.utils.DATABASE_NAME
import com.pti.tipapers.helpers.utils.TiPapersKonfigs
import com.pti.tipapers.ui.activities.base.BaseTiPapersActivity
import com.pti.tipapers.ui.activities.base.FavsDbManager
import com.pti.tipapers.ui.fragments.base.BaseDatabaseFragment
import com.pti.tipapers.ui.fragments.base.BaseTiPapersFragment
import com.pti.tipapers.ui.widgets.CustomToolbar
import com.pti.tipapers.viewmodels.FavoritesViewModel
...
abstract class TiPapersActivity : BaseTiPapersActivity<TiPapersKonfigs>(), FavsDbManager {
    
    override val prefs: TiPapersKonfigs by lazy { TiPapersKonfigs(this) }
    
    private val toolbar: CustomToolbar? by bind(R.id.toolbar)
    private val pager: ViewPager? by bind(R.id.pager)
    private var pagerAdapter: TiPapersSectionsAdapter? = null
    ...
    private fun initPagerAdapter() {
        pagerAdapter = TiPapersSectionsAdapter(
    ...
    override fun onCreateOptionsMenu(menu: Menu?): Boolean {
        menuInflater.inflate(R.menu.tipapers_menu, menu)
    ...
    private fun scrollToTop() {
        pagerAdapter?.get(lastSection)?.let {
            try {
                (it as? BaseTiPapersFragment<*, *>)?.scrollToTop()
    ...
    private fun doSearch(filter: String = "", closed: Boolean = false) {
        pagerAdapter?.get(lastSection)?.let {
            try {
                (it as? BaseTiPapersFragment<*, *>)?.enableRefresh(!filter.hasContent())
                synchronized(lock) {
                    postDelayed(150) {
                        (it as? BaseTiPapersFragment<*, *>)?.applyFilter(filter, closed)
                    }
                }
            } catch (ignored: Exception) {
    ...
    private fun refreshContent() {
        ...
        adapt?.let {
            pagerAdapter?.get(lastSection)?.let {
                try {
                    (it as? BaseTiPapersFragment<*, *>)?.reloadData(
