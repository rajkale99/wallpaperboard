...
internal val Fragment.configs: VPapersKonfigs
    get() = (activity as? ThemedActivity<VPapersKonfigs>)?.configs
        ?: activity?.let { VPapersKonfigs(it) }
        ?: context?.let { VPapersKonfigs(it) }
        ?: VPapersKonfigs(actv)
...
fun Context.clearDataAndCache() {
    ...
    clearCache()
    VPapersKonfigs(this).downloadsFolder = getString(
