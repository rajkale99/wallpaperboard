...
import com.viper.vpapers.ui.fragments.base.BaseDatabaseFragment
...
abstract class VPapersActivity : BaseVPapersActivity<VPapersKonfigs>(), FavsDbManager {
    
    override val prefs: VPapersKonfigs by lazy { VPapersKonfigs(this) }
    
    private val toolbar: CustomToolbar? by bind(R.id.toolbar)
    private val pager: ViewPager? by bind(R.id.pager)
    private var pagerAdapter: VPapersSectionsAdapter? = null
    ...
    private fun initPagerAdapter() {
        pagerAdapter = VPapersSectionsAdapter(
    ...
    override fun onCreateOptionsMenu(menu: Menu?): Boolean {
        menuInflater.inflate(R.menu.vpapers_menu, menu)
    ...
    private fun scrollToTop() {
        pagerAdapter?.get(lastSection)?.let {
            try {
                (it as? BaseVPapersFragment<*, *>)?.scrollToTop()
    ...
    private fun doSearch(filter: String = "", closed: Boolean = false) {
        pagerAdapter?.get(lastSection)?.let {
            try {
                (it as? BaseVPapersFragment<*, *>)?.enableRefresh(!filter.hasContent())
                synchronized(lock) {
                    postDelayed(150) {
                        (it as? BaseVPapersFragment<*, *>)?.applyFilter(filter, closed)
                    }
                }
            } catch (ignored: Exception) {
    ...
    private fun refreshContent() {
        ...
        adapt?.let {
            pagerAdapter?.get(lastSection)?.let {
                try {
                    (it as? BaseVPapersFragment<*, *>)?.reloadData(
